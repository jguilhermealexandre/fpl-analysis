/* The dashboard half of index.html.
 *
 * index.html is two documents in one file, and this is the one almost nobody
 * who arrives at easyfpl.com sees: the squad pitch, the expected-points
 * engine, the price ticker, the news strip, the readiness panel. It used to
 * ship with the landing page, where coverage put it at 2% used — 60KB parsed
 * and discarded before a first-time visitor could read the headline.
 *
 * Named in the manifest in index.html’s <head> alongside the twelve engine
 * scripts it calls, and fetched on the same two occasions they are: straight
 * away when a saved team makes the dashboard certain, on demand when someone
 * signs in from the landing page. activateTeamPage() in index-page.js waits
 * for it before calling loadTicker(), which is this file’s entry point.
 *
